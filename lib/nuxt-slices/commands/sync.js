...
export default async (/* config */c, args) => {
  try {
    ...
    const customTypes = await (async () => {
      const endpoint = Endpoints.CustomTypesApi.customTypes(config.default ? config.default.apiEndpoint : config.apiEndpoint);
      const _logHere = await CustomTypes.fetch(endpoint, cookies);
      console.log('Custom types return : ', _logHere);
      return _logHere;
    })();
    ...
    const [missingSlices, toFetchFromSlicemachine] = await (async () => {
      ...
      const slicemachineSlices = (await SliceMachine.fetchAllSlicesModels(endpoint, { framework: 'nuxt'}, cookies))
    ...
    if (toFetchFromSlicemachine) {
      const slicesZipPath = await (() => {
        ...
        const queryParams = { slices, framework: 'nuxt' };
