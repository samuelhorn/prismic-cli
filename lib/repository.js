...
async function createWithDomain(base, domain, args, customTypes, signedDocuments, users, noconfirm) {
  ...
  try {
    await (async () => {
      if (oauthAccessToken) return queryCreateRepositoryWithToken(base, domain, oauthAccessToken, customTypes, signedDocuments, users);
      const cookies = await Authentication.connect(base, args, noconfirm);
      return queryCreateRepositoryWithCookie(base, domain, cookies, customTypes, signedDocuments, users);
    })();

    const magicUrl = await Helpers.MagicLink.buildRedirectUrl(base, domain);
    Helpers.UI.display(`Redirecting you to: ${await Helpers.MagicLink.buildRedirectUrl(base, domain)} ...`);
