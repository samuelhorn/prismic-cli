...
function queryCreateRepositoryWithCookie({ base, domain, cookies, customTypes, signedDocuments, users, context }) {
  const params = { app: context };
  const url = `${base}/authentication/newrepository${Helpers.Url.buildQueryString(params)}`;
  const data = {
    domain,
    plan: 'personal',
    isAnnual: 'false',
...
function queryCreateRepositoryWithToken({ base, domain, token, customTypes, signedDocuments, users, context }) {
  const matches = base.match(/(https?:\/\/)(.*)/);
  const protocol = matches[1];
  const plateform = matches[2];
  const params = { access_token: token, app: context };
  const url = `${protocol}api.${plateform}/management/repositories?${Helpers.Url.buildQueryString(params)}`;
  const data = {
    domain,
    plan: 'personal',
    isAnnual: 'false',
...
async function createWithDomain({base, domain, args, customTypes, signedDocuments, users, noconfirm, context}) {
  const oauthAccessToken = args['--oauthaccesstoken'];

  try {
    await (async () => {
      if (oauthAccessToken) return queryCreateRepositoryWithToken({ base, domain, oauthAccessToken, customTypes, signedDocuments, users, context });
      const cookies = await Authentication.connect(base, args, noconfirm);
      return queryCreateRepositoryWithCookie({ base, domain, cookies, customTypes, signedDocuments, users, context });
    })();

    const magicUrl = await Helpers.MagicLink.buildRedirectUrl(base, domain);
    return { magicUrl, domain };
  } catch (error) {
...
async function isDomainAvailableOrRetry(newRepository, base, domain, args, noconfirm) {
  async function exec(d) {
    ...
    const isExist = await exists(base, d, args, noconfirm);
    if (isExist) {
      if (newRepository) {
