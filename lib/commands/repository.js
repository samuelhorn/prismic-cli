...
function queryCreateRepositoryWithCookie({ domain, customTypes, signedDocuments }) {
  const params = { app: ctx.appCtx };
  const url = `${ctx.base}/authentication/newrepository${Helpers.Url.buildQueryString(params)}`;
  const data = {
    domain,
    plan: 'personal',
    isAnnual: 'false',
...
function queryCreateRepositoryWithToken({ domain, customTypes, signedDocuments }) {
  ...
  const params = { access_token: ctx.Auth.oauthAccessToken, app: ctx.appCtx };
...
async function readZipAndCreateRepoWithCustomTypes(domain, template, folder, theme) {
  ...
  if (ctx.isNew) {
    ...
    const signedDocuments = readDocuments(tmpfolder);
...
async function create(templates, theme) {
  ...
  await readZipAndCreateRepoWithCustomTypes(selectedDomain, template, folder, theme);
